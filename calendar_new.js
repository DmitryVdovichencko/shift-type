...
console.log(rowTableCalc(monthArr(13,2018)));
...
const fillTable = (month,year,tableElement) => {
  ...
  createTable(rowTableCalc(monthObj),7,tableElement);//создаем таблицу для заполнения
   console.log(tableElement.childNodes[0].rows[0].cells[0]);
             ...
             for (let i = 0; i < 7; i++) {
              let day=week*7-monthObj.prevMonth.length+1+i;
                if(day<= monthObj.actMonth.length){
                  ...
                  console.log(i-(convertRuMonthDays(month,year).lastDateDay+1)); 
...
const calendarControl=(monthElement, yearElement, prevElement, nextElement,resultElement) =>{
  ...
  const control = (event) =>{
    ...
    if (event.target.classList.contains("next")){
      ...
      selectedDate.push(new Date(today.getFullYear(),today.getMonth(), event.target.textContent,-today.getTimezoneOffset()/60));
...
const outputResult = (outputEl, selectedDate, teamNum, typeNum,update) => {
  if(typeof(selectedDate)!=="undefined"){
    ...
    resultEl.innerHTML="<p>"+ selectedDate.toLocaleString("ru", options) + "</p>" + "Наша бригада " + shiftType(selectedDate,teamNum,typeNum);
  }
